# HUB75 display-frame plan

## Purpose

This is the operational source for current/future HUB75 display-frame work.
Completed functional history belongs in [CHANGELOG.md](../CHANGELOG.md).
Rationale is in [10-specification.md](10-specification.md), architecture in
[20-design.md](20-design.md), and verification in [30-verification.md](30-verification.md).

## Working method

1. inspect current source, open issues/PRs, live CI and generated evidence;
2. read affected component-local design documentation before non-trivial geometry changes;
3. derive the smallest current project step here rather than from old chat history;
4. correct this plan first when prerequisites/status changed;
5. inspect generated visual evidence when the question is visual;
6. keep digital verification distinct from physical acceptance.

Do not start dependent physical qualification or final integration while an
earlier physical interface gate remains open.

## Information sources

| Question | Authority |
| --- | --- |
| Current project step, gates and roadmap | this plan |
| Shared working/Git/PR/CI guidance | [`brainboxemb.meta/AGENTS.md`](https://github.com/brainboxemb/brainboxemb.meta/blob/main/AGENTS.md) |
| Shared SCAD conventions | [`brainboxemb.meta/domains/scad/README.md`](https://github.com/brainboxemb/brainboxemb.meta/blob/main/domains/scad/README.md) |
| Why the project exists | [10-specification.md](10-specification.md) |
| Application architecture/ownership | [20-design.md](20-design.md) |
| Detailed component construction | component-local `design/design.md` |
| Verification/current acceptance | [30-verification.md](30-verification.md) |
| Executable physical cases | [`vrf/physical/`](../vrf/physical/README.md) |
| Exact dependency/tool pins | `project.yml`, `project.scad.yml`, committed gitlinks and workflow callers |
...
| Completed functional history | [CHANGELOG.md](../CHANGELOG.md) |

## Current position

The v0.0.4 integration checkpoint is complete. Core couplers are digitally
accepted but physical panel fit remains open in issue #43, downstream of the
relevant real-panel validation in `lib.scad.hub75`.

Reinforcement/tube-mount/dovetail work is on main as explicit **WIP**. PR #45
is merged and later cleanup through PR #58 is merged. Standard core couplers
remain the default full-display assembly; WIP tube-aware couplers are opt-in.
Independent digital refinement may continue only while it does not silently
change unfrozen panel-facing core interfaces.

The old frame repository remains historical evidence only, not geometry authority.

## Current design direction

- five portrait panels with alternating rear-view data-chain orientation;
- two continuous horizontal aluminium reinforcement tubes;
- reusable clamp body from `lib.scad.clamps`;
- reusable locked sliding-dovetail interface from pinned `lib.scad.mechint`;
- project-owned placement/carrier geometry around public library APIs;
- ordinary axis-aligned inspection through `lib.scad.util`;
- Forge used where it clarifies project-owned modeling intent.
...
## Step 0 — Establish project-local handoff and forward plan

**Status:** complete — originally PR #40; entrypoint consolidated by Migration 010.

**Goal**

Make a fresh work session able to derive the next HUB75 project step from the repository itself.

The original PR introduced separate handoff/plan files. Migration 010 keeps the useful planning content but makes this `doc/00-plan.md` the single plan authority; historical paths now only point here.

**Exit criteria**

Met. README/AGENTS route here and generic working guidance routes to `brainboxemb.meta`.
...
**Acceptance record:** [Core coupler digital acceptance](../docs/02-core-coupler-digital-acceptance.md)
...
**Status:** complete on main — foundation merged through PR #45 and subsequent cleanup.
...
- add the pinned `lib.scad.mechint` as the source of the sliding-dovetail mating
...
**Status:** WIP on main — PR #45 is merged; later source/placement/CSG cleanup through PR #58 is also merged. Standard core couplers remain the default full assembly and tube-mount geometry remains opt-in.
...
- each interface position contains one short female dovetail with project-Z slide
  direction and local +Z top entry; the pinned `lib.scad.mechint` supplies the
  ...
  the coupler arm;
- the mating mechanism remains the pinned `lib.scad.mechint` and now uses its
...
- the integral female tongue follows the remaining host material behind the
  ...
  The shared opposing relief pockets leave a centered 0.8 mm flex web;
