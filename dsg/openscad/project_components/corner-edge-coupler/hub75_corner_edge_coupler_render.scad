...
//
// Explanatory construction states live here. They deliberately decompose the
// production geometry into visually understandable steps without changing the
// actual printable component implementation.
...
module _hub75_corner_edge_design_profile_outline_2d(
    coupler,
    line_width = 1.0
) {
    difference() {
...
module _hub75_corner_edge_design_reinforcement_guide_fragment(
    coupler,
    position
) {
    intersection() {
...
module _hub75_corner_edge_design_reinforcement_clearance_band(
    coupler,
    position
) {
    outer_d =
...
module _hub75_corner_edge_design_reinforcement_collision(
    coupler,
    position
) {
    intersection() {
        _hub75_corner_edge_design_reinforcement_guide_fragment(
            coupler,
            position
        );
...
module hub75_corner_edge_coupler_design(view = "final") {
    medium = hub75_corner_edge_coupler_create_for_size(
        side = "left",
        size = "medium"
    );
    large = hub75_corner_edge_coupler_create_for_size(
        side = "left",
        size = "large"
    );
    right_medium = hub75_corner_edge_coupler_create_for_size(
        side = "right",
        size = "medium"
    );
    ...
    if (view == "mating-reference") {
        ...
        color([0.43, 0.43, 0.43, 1.0])
            _hub75_corner_edge_design_reinforcement_panel_fragment(
                coupler,
                detail_position
            );
        ...
        color([0.88, 0.08, 0.06, 0.30])
            _hub75_corner_edge_design_reinforcement_clearance_band(
                coupler,
                detail_position
            );

        color([0.94, 0.03, 0.02, 0.96])
            _hub75_corner_edge_design_reinforcement_collision(
                coupler,
                detail_position
            );
