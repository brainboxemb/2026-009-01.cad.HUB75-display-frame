...
//
// Construction-only views live here so the design document can explain the
// geometry as a sequence of simple physical ideas without forcing the
// production source to use the same teaching sequence.
...
module _hub75_horizontal_edge_design_profile_window_2d(coupler) {
    ...
    outer = max(
        12,
        hub75_horizontal_edge_coupler_outer_projection(coupler) + 6
    );

    translate([0, (outer - reach) / 2])
        square([
            coupler.profile_size + 10,
            reach + outer
        ], center = true);
}
...
module _hub75_horizontal_edge_design_profile_outline_2d(
    coupler,
    line_width = 1.0
) {
    difference() {
...
module _hub75_horizontal_edge_design_reinforcement_guide_fragment(
    coupler,
    position
) {
    intersection() {
        _hub75_horizontal_edge_design_unrelieved_tall_guide(coupler);
        _hub75_horizontal_edge_design_reinforcement_crop(
            coupler,
            position
        );
    }
...
module _hub75_horizontal_edge_design_reinforcement_clearance_band(
    coupler,
    position
) {
    outer_d =
...
module _hub75_horizontal_edge_design_reinforcement_collision(
    coupler,
    position
) {
    intersection() {
        _hub75_horizontal_edge_design_reinforcement_guide_fragment(
            coupler,
            position
        );
...
module hub75_horizontal_edge_coupler_design(view = "final") {
    ...
    if (view == "mating-reference") {
        ...
        detail_position =
            _hub75_horizontal_edge_coupler_reinforcement_positions(coupler)[1];

        color([0.43, 0.43, 0.43, 1.0])
            _hub75_horizontal_edge_design_reinforcement_panel_fragment(
                coupler,
                detail_position
            );
        ...
        color([0.88, 0.08, 0.06, 0.30])
            _hub75_horizontal_edge_design_reinforcement_clearance_band(
                coupler,
                detail_position
            );

        color([0.94, 0.03, 0.02, 0.96])
            _hub75_horizontal_edge_design_reinforcement_collision(
                coupler,
                detail_position
            );
