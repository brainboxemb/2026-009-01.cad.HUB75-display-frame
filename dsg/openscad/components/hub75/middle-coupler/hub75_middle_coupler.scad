...
/* [Preview] */
c_view = "final"; // [final,functional,profile,base,screw-holes,tube-pockets,guides,reinforcement-locators,seam-locator,reference-pockets,center-marks]
...
module _hub75_middle_coupler_screw_cutters(coupler_obj) {
    ...
    for (x = hub75_middle_coupler_screw_x_positions(coupler_obj)) {
        fg_xf_frame(
            pos_mm = [x, coupler_obj.base_thickness, 0],
            x_axis = [1, 0, 0],
            z_axis = [0, -1, 0]
        ) {
            fg_cut_cylinder(
                diameter_mm = coupler_obj.screw_hole_diameter,
                height_mm = coupler_obj.base_thickness,
...
module _hub75_middle_coupler_rib_cross_keepout_2d(coupler_obj) {
    ...
    union() {
        ...
        for (sx = [-1, 1], sz = [-1, 1])
            scale([sx, sz])
                fg_xf_xzmove([half_vertical, half_horizontal])
                    difference() {
...
module _hub75_middle_coupler_reinforcement_relief_cutters(coupler_obj) {
    ...
    for (position_xz_mm =
        ...
        fg_xf_frame(
            ...
            fg_cut_cylinder(
                diameter_mm = relief_diameter,
                height_mm = coupler_obj.guide_height,
                overlap = [FG_BOTTOM(), FG_TOP()],
                overlap_mm = _HUB75_MIDDLE_COUPLER_EPS
            );
}
...
module _hub75_middle_coupler_reinforcement_locator(coupler_obj) {
    ...
    fg_xf_frame(
        pos_mm = [0, _HUB75_MIDDLE_COUPLER_EPS, 0],
        x_axis = [1, 0, 0],
        z_axis = [0, -1, 0]
    )
        cylinder(
    ...
    fg_xf_frame(
        pos_mm = [0, -pad_h + _HUB75_MIDDLE_COUPLER_EPS, 0],
        x_axis = [1, 0, 0],
        z_axis = [0, -1, 0]
    )
        cylinder(
...
_preview_coupler_obj =
    hub75_middle_coupler_create(
        ...
        mounting_tube_radial_clearance =
            d_mounting_tube_radial_clearance_mm,
        mounting_tube_axial_clearance =
            d_mounting_tube_axial_clearance_mm,
        reinforcement_bushing_clearance =
            d_reinforcement_bushing_clearance_mm,
        reinforcement_locator_pad_radial_clearance =
