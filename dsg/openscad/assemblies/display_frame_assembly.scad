...
function _hub75_display_frame_panel_pitch_x(panel_obj) =
    hub75_p5_64x32_panel_nominal_width(panel_obj);

function _hub75_display_frame_half_height(panel_obj) =
    hub75_p5_64x32_panel_nominal_height(panel_obj) / 2;

function _hub75_display_frame_half_width(panel_obj, panel_count = 5) =
    panel_count * _hub75_display_frame_panel_pitch_x(panel_obj) / 2;

function _hub75_display_frame_seam_x(panel_obj, seam_index, panel_count = 5) =
    -_hub75_display_frame_half_width(panel_obj, panel_count)
    + (seam_index + 1) * _hub75_display_frame_panel_pitch_x(panel_obj);

function hub75_display_frame_reference_width(panel_obj, panel_count = 5) =
    panel_count * hub75_p5_64x32_panel_nominal_width(panel_obj) + 40;

function hub75_display_frame_reference_height(panel_obj) =
    hub75_p5_64x32_panel_nominal_height(panel_obj) + 40;

function hub75_display_frame_tube_length(panel_obj, panel_count = 5) =
    hub75_display_frame_reference_width(panel_obj, panel_count);
...
module _hub75_display_frame_corner_couplers(
    ...
    edge_x = _hub75_display_frame_half_width(panel_obj, panel_count);
    edge_z = _hub75_display_frame_half_height(panel_obj);
...
module _hub75_display_frame_tube_clamps(
    ...
    horizontal_positions =
        hub75_tube_horizontal_edge_clamp_positions_mm(horizontal_coupler_obj);
    left_x = hub75_tube_corner_edge_clamp_x_mm(left_coupler_obj);
    right_x = hub75_tube_corner_edge_clamp_x_mm(right_coupler_obj);

    horizontal_clamp_obj =
        hub75_tube_clamp_create(
            dovetail_obj =
                hub75_tube_mount_dovetail_create(
                    host_depth_mm = horizontal_coupler_obj.base_thickness
                )
        );
    left_clamp_obj =
        hub75_tube_clamp_create(
            dovetail_obj =
                hub75_tube_mount_dovetail_create(
                    host_depth_mm = left_coupler_obj.base_thickness
                )
        );
    right_clamp_obj =
        hub75_tube_clamp_create(
            dovetail_obj =
                hub75_tube_mount_dovetail_create(
                    host_depth_mm = right_coupler_obj.base_thickness
                )
        );
...
module _hub75_display_frame_aluminium_tubes(
    ...
    edge_z = _hub75_display_frame_half_height(panel_obj);
    length = hub75_display_frame_tube_length(panel_obj, panel_count);
    x_min = -length / 2;
    clamp_obj = hub75_tube_clamp_create();
    tube_obj =
        aluminium_tube_create(
            length_mm = length,
            outer_diameter_mm = clamp_obj.base_clamp.tube_diameter,
            wall_thickness_mm = 1
        );
    tube_y =
        mounting_y
...
module _hub75_display_frame_debug_reference(panel_obj, panel_count) {
    width = hub75_display_frame_reference_width(panel_obj, panel_count);
    height = hub75_display_frame_reference_height(panel_obj);

    hub75_reference_box_wireframe(
        x_min = -width / 2,
        x_max = width / 2,
        y_min = 0,
        y_max = hub75_p5_64x32_panel_mounting_plane_y(panel_obj),
        z_min = -height / 2,
        z_max = height / 2
    );
}
...
module hub75_display_frame_assembly(
    ...
    active_middle_coupler_obj =
        is_undef(middle_coupler_obj)
            ? hub75_middle_coupler_create_for_size(
                size = coupler_size,
                panel_obj = panel_obj
            )
            : middle_coupler_obj;
    active_horizontal_coupler_obj =
        is_undef(horizontal_coupler_obj)
            ? hub75_horizontal_edge_coupler_create_for_size(
                size = coupler_size,
                panel_obj = panel_obj
            )
            : horizontal_coupler_obj;
    active_left_corner_coupler_obj =
        is_undef(left_corner_coupler_obj)
    ...
    active_right_corner_coupler_obj =
        is_undef(right_corner_coupler_obj)
            ? hub75_corner_edge_coupler_create_for_size(
                side = "right",
                size = coupler_size,
                panel_obj = panel_obj
    ...
    if (debug_reference_visible)
        _hub75_display_frame_debug_reference(panel_obj, panel_count);
}
