...
![Mating Reference](img/01-mating-reference.png)
...
![Profile Horizontal Arm](img/02-profile-horizontal-arm.png)
...
![Profile Vertical Arm](img/03-profile-vertical-arm.png)
...
![Profile Raw Plus](img/04-profile-raw-plus.png)
...
This raw rectangular PLUS is an explanatory construction state rather than a
separate production module. Production code goes directly from the derived arm
dimensions to the rounded polygon shown in the next step.
...
![Profile Rounded](img/05-profile-rounded.png)
...
![Screw Holes](img/07-screw-holes.png)

The production subtraction is owned by:

```scad
_hub75_middle_coupler_screw_cutters(coupler)
```

That module loops over the panel-derived local screw positions returned by:

```scad
hub75_middle_coupler_screw_x_positions(coupler)
```

and creates the through bore plus the two shallow face-relief cylinders. The
next base stage applies those cutters with:
...
![Tube Pockets](img/08-tube-pockets.png)

The actual cylindrical pocket volumes are generated by:

```scad
_hub75_middle_coupler_mounting_tube_pocket_cutters(coupler)
```

and are subtracted from the already drilled base in:
...
![Guide Keepout](img/09-guide-keepout.png)

This is the same authoritative panel keep-out introduced in step 1, now used as
a Boolean construction input:
...
![Guide Raw Shell](img/10-guide-raw-shell.png)
...
![Guide Rounded Shell](img/11-guide-rounded-shell.png)

The production wrapper that applies this operation is:

```scad
_hub75_middle_coupler_guide_shell_2d(coupler)
```

It falls back to the raw shell when no effective rounding can safely be applied.
...
![Full coupler guide reinforcement relief construction](img/12-guide-reinforcement-reliefs.png)
...
![HUB75 reinforcement and guide clearance detail](img/13-guide-reinforcement-detail.png)
...
Its key derived dimension is computed directly from the panel-derived outer
diameter plus coupler-owned print clearance:
...
Inside that module, `_hub75_middle_coupler_reinforcement_positions(coupler)`
places the same cylindrical cutter at the two mirrored panel reinforcement
positions. Step 13 shows where those cutters are subtracted from the raised guide
solid.
...
![Reinforcement Locators](img/15-reinforcement-locators.png)

Those dimensions are derived rather than hard-coded into the locator module:
...
The single physical pad/pin stack is built by:

```scad
_hub75_middle_coupler_reinforcement_locator(coupler)
```

and the two panel positions are populated by:

```scad
_hub75_middle_coupler_reinforcement_locators(coupler)
```

The physical panel recess/hole dimensions come from the reusable HUB75 API.
Only the radial/axial clearances and pin insertion length are coupler-owned
design choices.
...
The two production accessors make those two stages explicit:
...
The actual 3D locator is built by:

```scad
_hub75_middle_coupler_seam_locator(coupler)
```

That module takes the wider rear width, evaluates the narrower width at the end
of the available panel taper and forms the transition between those sections. If
the configured locator is deeper than the taper region, the remaining depth
continues at the final narrow width.

![Seam Locator](img/16-seam-locator.png)
...
![Functional](img/17-functional.png)

This intermediate production assembly is explicit in the source:

```scad
_hub75_middle_coupler_functional_build(coupler)
```

Conceptually that module combines the exact helpers developed in the preceding
steps:
...
Keeping this as an explicit design and source stage separates **mechanical mating
geometry** from markings that exist only to aid inspection and measurement.
...
The source makes that depth rule explicit through:
...
thickness and snapped to a 2.5 mm reference grid:
...
Two symmetric lanes are used only when they fit safely in both arms. The private
position helper turns those choices into the complete centre list:

```scad
_hub75_middle_coupler_reference_pocket_positions(coupler)
```

![Reference Pockets](img/18-reference-pockets.png)

The 3D subtraction is performed by:

```scad
_hub75_middle_coupler_reference_pocket_cutters(coupler)
```

That cutter keeps the visible part cylindrical at Ø3, adds only the short Ø3→Ø2
taper at the bottom, and clips the complete cutter set against an inset of the
real rounded PLUS profile so the pockets cannot break through an outside edge.
...
The 2D mark generator creates the centre cross and alternating 5/10 mm tick
lengths, then removes keep-out discs around the structural screw lands:

```scad
_hub75_middle_coupler_center_marks_2d(coupler)
```

The result is clipped against an inset of the actual rounded PLUS outline by:

```scad
_hub75_middle_coupler_center_mark_pattern_2d(coupler)
```

![Center Marks](img/19-center-marks.png)

Finally the shallow engraving volume is generated by:

```scad
_hub75_middle_coupler_center_mark_cutters(coupler)
```

That module limits engraving depth relative to the base thickness and subtracts
the clipped 2D pattern only from the visible rear face.

## 19. Complete middle coupler

The final base-with-surface-details stage subtracts both reference-detail cutter
families from the mechanically prepared base:

```scad
_hub75_middle_coupler_base_with_surface_details(coupler)
```

The public build then combines that detailed base with the positive mating
geometry and owns the configured render resolution:
